...
func SliceToString(inParam []interface{}, interval string) string {
    ...
    for i, val := range inParam {
        ...
        case float32:
            outStr += strconv.FormatFloat(float64(val.(float32)), 'f', -1, 64)
            if i < len(inParam)-1 {
                outStr += interval
            }
        case float64:
            outStr += strconv.FormatFloat(val.(float64), 'f', -1, 64)
            if i < len(inParam)-1 {
                outStr += interval
            }
        }
...
func (stMyInterfaceConv) GetValToSliceInt(in interface{}) []int {
    ...
    if inList, ok := in.([]interface{}); ok {
        for _, val := range inList {
            ...
            //}
            outList = append(outList, CGetInterface.GetInt(val))
        }
    }
    return outList
}
func (stMyInterfaceConv) GetValToSliceInt64(in interface{}) []int64 {
    ...
    if inList, ok := in.([]interface{}); ok {
        for _, val := range inList {
            ...
            //	outList = append(outList, x)
            //}
            outList = append(outList, CGetInterface.GetInt64(val))
        }
    }
    return outList
}
func (stMyInterfaceConv) GetValToSliceString(in interface{}) []string {
    ...
    if inList, ok := in.([]interface{}); ok {
        for _, val := range inList {
            ...
            //    outList = append(outList, x)
            //}
            outList = append(outList, CGetInterface.GetString(val))
        }
    }
    return outList
}
